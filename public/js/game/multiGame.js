...
//function.check answers in db
// const checkAnswers = (answers) =>{
//     return new Promise  ((resolve,reject)=>{


//         let answersArr = [];
//         // console.log('moji odgovori su: ' + answers[0])
//         // console.log('Protivnikovi odgovori su: ' + answers[1])
//         category.forEach((elem,index)=>{
//             // let firstLetter = checkFirstLetter(answers[index])
//             db.collection("pojmovi")
//             .where("kategorija", "==", `${elem}`)
//             // .where("pocetnoSlovo","==", `${firstLetter}`)
//             .where("pojam", "==", `${answers[index]}`)
//             .get()
//             .then((querySnapshot) => {
//                 if (querySnapshot.size > 0) {
//                     answersArr.push(answers[index])
//                 }            
//                 else{
//                     answersArr.push('')
//                 }
...
const renderResult = (data) =>{
    category.forEach((elem, index)=>{
        ...
        //Check user sa if proveri dal je isti user kao u local toru ako je isti onda stamaapja drugog ako je razlicit onda njega 
        finalScore.children[0].innerHTML = `<p>${localStorage.getItem(('username'))}: <span>${userPoints}</span></p>`;
        finalScore.children[1].innerHTML = `<p>${data[1].answ.name}: <span>${opponentPoints}</span></p>`;
        if(elem === "Predmet"){
            declareWinnerAlert (userPoints, opponentPoints, `Kompjuter je pobedio!!!!`)
        }
        })
        console.log(userPoints, opponentPoints)
